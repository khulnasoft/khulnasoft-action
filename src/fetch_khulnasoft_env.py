"""
# Purpose:
Wait for the Khulnasoft environment to be ready,
then put its data in the environment.
# Required environment variables:
- GITHUB_ENV
- GITHUB_REPOSITORY
- GITHUB_REF_NAME
- INPUT_API_TOKEN or KHULNASOFT_API_TOKEN
- INPUT_TIMEOUT_MINUTES or KHULNASOFT_TIMEOUT
- INPUT_APP_NAME or KHULNASOFT_APP_NAME
"""
...
if not api_token:
    exit("No KHULNASOFT_API_TOKEN provided, exiting.")

# Get the timeout
timeout_minutes = os.environ.get("INPUT_TIMEOUT_MINUTES")
if not timeout_minutes:
    timeout_minutes = os.environ.get("KHULNASOFT_TIMEOUT")
if timeout_minutes:
    try:
        timeout_minutes = int(timeout_minutes)
    except Exception:
        exit(
            'ERROR: the KHULNASOFT_TIMEOUT provided ("{}") is not an integer'.format(
                timeout_minutes
            )
        )
else:
...
if not app_name:
    app_name = os.environ.get("KHULNASOFT_APP_NAME")
...
def fetch_khulnasoft_environment():
    """Fetch the Khulnasoft environment for this Github job"""

    # Hit the Khulnasoft API
    try:
        args = {
    ...
    if not len(response["data"]):
        exit("ERROR: no matching Khulnasoft environment found")
...
def restart_environment(environment_id):
    """Restart the Khulnasoft environment with the provided ID"""
...
def wait_for_environment():
    """Return the Khulnasoft environment data once it's ready"""

    auto_restart = True
    was_restarted = False

    # Check the environment
    environment_id, environment_data = fetch_khulnasoft_environment()
    ...
    while not environment_data["ready"]:
        ...
        if all([environment_data["retired"], auto_restart, not was_restarted]):
            ...
            was_restarted = True
            print("Restarted Khulnasoft environment...")
        ...
        print("Waiting for Khulnasoft environment...{}".format(wait_string))
        time.sleep(15)
...
def main():
    ...
    with open(bash_env_path, "a") as bash_env:
        bash_env.write(
            "\n".join(
                [
                    "KHULNASOFT_BYPASS_TOKEN={}".format(environment_data["bypass_token"]),
                    "KHULNASOFT_ENVIRONMENT_ID={}".format(environment_id),
                    "KHULNASOFT_ENVIRONMENT_URL={}".format(environment_data["url"]),
                    "KHULNASOFT_ENVIRONMENT_READY={}".format(environment_data["ready"]),
                    "KHULNASOFT_ENVIRONMENT_RETIRED={}".format(
                        environment_data["retired"]
                    ),
                ]
                + ["KHULNASOFT_ENVIRONMENT_COMMIT_HASH={}".format(commit_hash)]
                if commit_hash
                else []
            )
        )

    print("Khulnasoft environment data written to {}!".format(bash_env_path))
